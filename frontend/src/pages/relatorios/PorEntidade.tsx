...
export function PorEntidade() {
  ...
  const gerar = async (e: React.FormEvent) => {
    ...
    setErro(null)
    try {
      const resultado = await relatoriosApi.porEntidade({
  ...
  return (
    <div className="page max-w-4xl">
      <h1 className="page-title mb-4">Relatório por Entidade</h1>
      ...
      <form onSubmit={gerar} className="panel mb-6 grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="field">
          Tipo de Entidade
          <select className="input" value={tipoEntidade} onChange={(e) => setTipoEntidade(e.target.value)}>
            <option value="">Todos</option>
            {lookups.enums.tiposEntidade.map((t) => (
              <option key={t.sigla} value={t.sigla}>{t.descricao}</option>
            ))}
          </select>
        </label>
        <label className="field">
      ...
      {linhas && <ResultTable headers={['Razão Social', 'Tipo Entidade', 'Tipo Contrato', 'Qtde', 'Valor total']} rows={linhas} />}
    </div>
  )
}
